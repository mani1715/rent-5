...
const ChatModal = ({ isOpen, onClose, conversation, ownerId, listingId }) => {
  ...
  const loadMessages = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/conversations/${currentConversation._id}/messages`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const markAsRead = async () => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}/api/conversations/${currentConversation._id}/read`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (socket && connected) {
  ...
  const sendMessage = async (e) => {
    ...
    const receiverId = currentConversation.participants.find(p => p._id !== user._id)?._id;
